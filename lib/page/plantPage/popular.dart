...
final PopularProductsProvider =
    StateNotifierProvider<PopularNotifier, AsyncValue<List<Plant>>>(
      (ref) => PopularNotifier(),
    );

class PopularNotifier extends StateNotifier<AsyncValue<List<Plant>>> {
  PopularNotifier() : super(const AsyncLoading()) {
    loadPopular();
  }
  ...
  Future<void> loadPopular() async {
    try {
      ...
      print('🔥 Popular Offers Converted: ${plants.length} plants');
      state = AsyncData(plants);
    } catch (e, st) {
      print('❌ Error loading Popular offers: $e');
      print(st);
...
class _PopularPageState extends ConsumerState<PopularPage>
  ...
  Future<void> _handleRefresh() async {
    ...
    await ref.read(PopularProductsProvider.notifier).refresh();
  }
